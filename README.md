...
## 개발 방법론 (How This Was Built)

이 프로젝트는 Claude Code(Anthropic의 AI 코딩 도구)를 이용해 
대화 형식으로 개발했습니다. 데이터 생성 로직, 모델링 코드, 
대시보드 UI 등 실제 코드는 대부분 AI가 작성했습니다.

제가 한 역할은 원래 작성했던 연구계획서의 설계(측정 항목, 
시점 구조, 모델링 방법론, 문헌 근거)를 각 개발 단계마다 
요구사항으로 전달하고, 결과물이 계획서의 의도와 맞는지 확인하는 
것이었습니다. 코드를 직접 작성하거나 디버깅하지는 않았습니다.

즉 이 프로젝트는 "AI 코딩 도구를 활용해 연구계획서의 아이디어를 
동작하는 프로토타입으로 빠르게 구현해본 것"이며, 코드 구현 역량 
자체를 증명하기 위한 것이 아니라 문제 설계와 AI 도구 활용 능력을 
보여주기 위한 것입니다.
